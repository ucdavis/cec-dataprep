...
export const processCluster = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    await osrm.nearest(options, async (err, response) => {
      ...
      const mean_yarding_meters =
        0.002 * centerOfBiomassDistanceToLanding ** 2 +
        0.044 * centerOfBiomassDistanceToLanding +
        137.5815; // all in meters
      const mean_yarding = mean_yarding_meters * metersToFeetConstant; // final conversion to feet
      ...
      if (
        firstCluster.Land_Ownership !== null &&
        firstCluster.Land_Ownership !== undefined &&
        firstCluster.Land_Ownership !== ''
      ) {
